...
function launchAdmin(nextFunction) {
  sendFetchRequest(
    "GET",
    "https://getdagnis.github.io/2020/public/portfolio_full.json"
  )
...
function portfolioMain(orderedPortfolio) {
  ...
  for (i = 0; i < objectKeys.length; i++) {
    ...
    let thisImageDiv = "";
    let allImageDivs = `
            <div class="viewer-img first" style="background: url('works/${
              thisObj.key
            }/${thisObj.images[0]}') no-repeat center;
            background-size: cover;">
                <div class="viewer-description">
                    <div class="description-left">
                        <div class="description-top">
                            <h2 class="${thisObj.key}-name ${
      thisObj.key
    } name edit-ready">${thisObj.name}</h2>
                            <h3 class="${thisObj.key}-title ${
      thisObj.key
    } title edit-ready">${thisObj.title}</h3>
                            <h3 class="${thisObj.key}-work ${
      thisObj.key
    } work edit-ready">${thisObj.work}</h3>
                            <h3 class="${thisObj.key}-year ${
      thisObj.key
    } year edit-ready">${thisObj.year}</h3>
                        </div>
                        <div class="description-bottom">
                            <p class="${thisObj.key}-description ${
      thisObj.key
    } description edit-ready">${thisObj.description}</p>
                        </div>
                    </div>
                    <div class="description-right">
                        <img class="thumb descr-thumb" src="img/thumbs/${
                          thisObj.key
                        }.svg" alt="${thisObj.name + ", " + thisObj.work}" />
                    </div>
    ...
    templateHTML = `
        <div class="grid-item" id="grid-item-${thisObj.key}">
            <div class="grid-item-inner ${thisRow} ${thisCol} itemBounce" style="animation-delay: ${animationDelay}s">
                <img class="thumb" src="img/thumbs/${thisObj.key}.svg" alt="${
      thisObj.name + ", " + thisObj.work
    }" />
                <div class="item-inner ${
                  thisObj.key
                } ${thisRow} ${thisCol} ${rowAndCol} ${whichInnerSide}">
        ...
        <div class="expanded ${thisObj.key} expand-${expandRow} scale-0 hidden">
            <div class="viewer" id="${thisObj.key}-viewer">
                <div class="images ${thisObj.images.length}" id="${
      thisObj.key
    }-images">
                    ${allImageDivs}
...
function orderPortfolioByPositions(fetchData, startPortfolio) {
  ...
  if (orderedPortfolio.length !== desiredLength) {
    console.warn(
      "Warning:",
      desiredLength - orderedPortfolio.length,
      "projects missing after sorting"
    );
